...
export async function POST(req: NextRequest) {
  try {
    ...
    if (checkout.estado === "capturada") return NextResponse.json({ success: true, orderId: checkout.orderId });
    ...
    const result = await db.runTransaction(async (transaction) => {
      ...
      if (freshData?.estado === "capturada") return { orderId: freshData.orderId };
